...
sudo pacman -S fcitx5-im fcitx5-configtool fcitx5-gtk fcitx5-qt fcitx5-hangul sddm  
...
yay -S --needed --noconfirm wezterm-git lua-language-server sddm-theme-sugar-candy
mkdir -p ~/.config/tmux
mkdir -p ~/.config/.tmux
# nvim
...
sudo cp $PWD/sddm.conf /etc/sddm.conf
